...
import { PRINCIPLES, PRINCIPLES_DE } from './constants';
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] md:text-[12px] uppercase tracking-[0.3em] md:tracking-[0.4em] text-blue-500 font-extrabold mt-1 md:mt-1.5 flex items-center gap-2">
              <Medal className="w-3 h-3 md:w-4 md:h-4" /> {t.appTagline}
            </span>
          </div>
        </div>
        ...
        <div className="bg-white px-5 py-3 md:px-7 md:py-4 rounded-[1.5rem] md:rounded-[2.5rem] shadow-sm border border-blue-100 flex items-center gap-4 md:gap-6 group transition-all hover:shadow-md">
          ...
          <div className="flex flex-col min-w-0">
            <div className="text-[9px] font-black uppercase text-gray-400 tracking-[0.15em] mb-0.5 flex items-center gap-1.5">
              <Award className="w-2.5 h-2.5 text-blue-400" /> {t.masteryLabel}
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xl md:text-2xl font-black text-gray-900 tracking-tight">{mastered.size}/{principles.length}</span>
              <span className="text-[9px] font-black px-2 py-0.5 bg-yellow-400 text-yellow-900 rounded-full shadow-sm uppercase tracking-widest truncate">{t.masteryBadges}</span>
            </div>
          </div>
        </div>
        </div>
      ...
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-xl border-y border-blue-100/50 shadow-sm">
        <div className="max-w-6xl mx-auto flex flex-col">
            ...
            <div className="py-1.5 px-2 flex items-center justify-center gap-2 md:gap-8 border-t border-blue-50 bg-blue-50/20">
               {[
                 { id: 'overview', label: t.navBrief, icon: <Binoculars className="w-3.5 h-3.5" /> },
                 { id: 'playground', label: t.navPlay, icon: <Gamepad2 className="w-3.5 h-3.5" /> },
                 { id: 'code', label: t.navCode, icon: <Terminal className="w-3.5 h-3.5" /> }
               ].map((section) => (
                 <button
                    key={section.id}
      ...
      <main className="max-w-6xl mx-auto px-4 pt-8 md:pt-12">
                ...
                <div className={`rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 ${currentPrinciple.bgLight} border ${currentPrinciple.borderColor} shadow-xl shadow-black/5 overflow-hidden relative`}>
                    ...
                    <div className="flex items-center justify-between mb-6 md:mb-8">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[9px] md:text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mt-1.5 md:mt-2 bg-white/50 px-2 py-0.5 rounded-full inline-block">{t.cardChief}: {currentPrinciple.pupName}</span>
                        </div>
                      ...
                      {currentPrinciple.learnMoreLinks && currentPrinciple.learnMoreLinks.length > 0 && (
                        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-4 md:p-5 rounded-2xl md:rounded-3xl border border-blue-100 shadow-sm">
                          <div className="flex items-center gap-2 mb-3 md:mb-4">
                            ...
                            <span className="text-[9px] md:text-[10px] font-extrabold uppercase tracking-widest text-blue-700">{t.cardLearnMore}</span>
                          </div>
                    ...
                    <div className="mt-6 md:mt-8 bg-yellow-400/10 p-4 md:p-5 rounded-2xl md:rounded-3xl border border-yellow-400/20 shadow-sm flex gap-3 md:gap-4 transform hover:scale-[1.01] transition-transform">
                        ...
                        <div className="min-w-0">
                            <span className="text-[9px] md:text-[10px] font-extrabold uppercase tracking-widest text-yellow-700">{t.cardProTip}</span>
                ...
                <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] shadow-lg shadow-black/5 border border-gray-100 overflow-hidden transition-all duration-300">
                    ...
                    >
                      <div className="flex items-center gap-3 md:gap-4 min-w-0">
                        ...
                        <div className="flex flex-col min-w-0">
                          <h4 className="font-extrabold text-gray-800 text-[10px] md:text-[12px] uppercase tracking-[0.1em] md:tracking-[0.2em] leading-none truncate">{t.failureTitle}</h4>
                          <span className={`text-[8px] md:text-[9px] font-bold mt-1 md:mt-1.5 uppercase tracking-widest transition-colors truncate ${showFailures ? 'text-red-600' : 'text-gray-400'}`}>
                            {showFailures ? t.failureSubOpen : t.failureSubClosed}
                          </span>
                        </div>
                      </div>
            ...
            <div className="lg:col-span-2 space-y-12 md:space-y-16">
                    ...
                    <div className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8 px-1 md:px-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                            <h3 className="text-2xl md:text-3xl font-extrabold text-gray-800 tracking-tight">{t.missionTraining}</h3>
                            {mastered.has(currentPrinciple.id) && (
                              <div className="w-fit flex items-center gap-1.5 bg-yellow-400/20 text-yellow-700 px-2.5 py-1 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest border border-yellow-400/30 animate-in zoom-in">
                                <Star className="w-2.5 h-2.5 fill-yellow-500" /> {t.masteryEarned}
                              </div>
                            )}
                          </div>
                          <p className="text-[9px] md:text-[10px] font-black uppercase text-gray-400 tracking-widest mt-0.5">{t.missionTrainingSubtitle}</p>
                        </div>
                    </div>
                ...
                <section id="code" aria-label="Code Blueprints Area" className="scroll-mt-32 pb-8 md:pb-12">
                     <div className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8 px-1 md:px-2">
                        ...
                        <div className="min-w-0">
                          <h3 className="text-2xl md:text-3xl font-extrabold text-gray-800 tracking-tight">{t.missionBlueprints}</h3>
                          <p className="text-[9px] md:text-[10px] font-black uppercase text-gray-400 tracking-widest mt-0.5">{t.missionBlueprintsSubtitle}</p>
                        </div>
                    </div>
        ...
        <section className="mt-16 md:mt-24 pt-12 md:pt-16 border-t border-gray-200" aria-label="Debriefing Summary">
          <div className="flex items-center gap-4 md:gap-5 mb-8 md:mb-12">
            ...
            <div>
              <h3 className="text-3xl md:text-4xl font-black text-gray-900 tracking-tighter">{t.debriefingTitle}</h3>
              <p className="text-[10px] md:text-[11px] text-gray-400 font-black mt-1 uppercase tracking-[0.15em] md:tracking-[0.25em]">{t.debriefingSubtitle}</p>
            </div>
          </div>
              ...
              <table className="w-full text-left border-collapse min-w-[700px]">
                <thead>
                  <tr className="bg-gray-50/50 border-b border-gray-200">
                    <th className="px-8 md:px-10 py-6 md:py-8 text-[10px] md:text-[11px] font-black text-gray-400 uppercase tracking-widest">{t.tableColUnit}</th>
                    <th className="px-8 md:px-10 py-6 md:py-8 text-[10px] md:text-[11px] font-black text-gray-400 uppercase tracking-widest">{t.tableColSpec}</th>
                    <th className="px-8 md:px-10 py-6 md:py-8 text-[10px] md:text-[11px] font-black text-gray-400 uppercase tracking-widest">{t.tableColDeploy}</th>
                    <th className="px-8 md:px-10 py-6 md:py-8 text-[10px] md:text-[11px] font-black text-gray-400 uppercase tracking-widest">{t.tableColRisk}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {principles.map((row, i) => (
                    <tr key={i} className="hover:bg-blue-50/30 transition-all group">
                      ...
                      <td className="px-8 md:px-10 py-5 md:py-7 text-[13px] md:text-[14px] text-gray-400 font-bold">{t.tableColRiskValue}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <footer className="py-16 md:py-20 bg-gray-900 text-center relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4">
          <p className="text-gray-400 text-[12px] md:text-sm font-bold leading-relaxed px-4">
              {t.footerInspiredBy} <a href="https://dev.to/suckup_de/solid-principles-from-paw-patrol-to-the-neighborhood-1008" className="text-blue-400 hover:text-blue-300 underline underline-offset-8 decoration-2 decoration-blue-400/30 transition-all inline-flex items-center gap-1" target="_blank" rel="noopener noreferrer">{t.footerInspiredByLink} <ExternalLink className="w-3 h-3" /></a>
          </p>
          ...
          <div className="mt-6 md:mt-8">
            ...
            >
              ...
              <span className="uppercase tracking-wider">{t.footerContribute}</span>
